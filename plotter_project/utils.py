...
jet_attributes_global = [
        "pt", "eta", "phi", "m", "puid", "jetid", "deepflavB", "hadronFlavour"] #nanov9
...
def define_jets_with_minimum_selection_for_histos(samples, is_bstautau, bstautau_conditions, part_samples):
    ...
    if is_bstautau:
        for attr in jet_attributes:
            samples = samples.Define(f"selected_jets_for_histo_{attr}", f"selected_jets_{attr}")#[{bstautau_conditions['general']}]") # here consider only the jets matching with the bs->tautau decay
